...
var defaultEncoding = localStorage.getItem('insight-encoding') || "cashAddr";

var to_cashaddr = function (addr){
  try {
    const hash = myBase.decode(addr);
    if (hash.length !== 1 + 20) {
    ...
    if (prefix == 0x00){
      type_encode = "P2PKH";
      network_encode = "bitcoincash";
    } else if (prefix == 0x05) {
      type_encode = "P2SH";
      network_encode = "bitcoincash";
    } else if (prefix == 0x6f) {
      type_encode = "P2PKH";
      network_encode = "bchtest";
    }else if (prefix == 0xc4) {
      type_encode = "P2SH";
      network_encode = "bchtest";
    }

    if (network_encode == "" || type_encode == "") {
      throw new TypeError('Address has mismatched network/type value.');
    }

    var hashhex = [];
    hash.slice(1).forEach(function (valor, indice, array) {
        hashhex[indice] = valor;
    });

    const address = myBundle.encode(network_encode, type_encode, hashhex);
    return address;
  } catch (e) {
    if (addr.length == 42) {
      //missing prefix
      var prefix = "bitcoincash:";
      addr = prefix.concat(addr);
    }
    return addr;
  }
};

var to_legacy = function (addr){
  try {
    if (addr.length == 42) {
      //missing prefix
      var prefix = "bitcoincash:";
    ...
    const decoded = myBundle.decode(addr);

    var network = "mainnet";
    if (decoded.prefix == "bchtest" || decoded.prefix == "BCHTEST") {
      network = "testnet";
    }

    if (decoded.hash.length == 20){
      var hashhex = new Uint8Array(21);
      decoded.hash.forEach(function (valor, indice, array) {
        hashhex[indice+1] = valor;
      });      
      hashhex[0] = 0x00;
      if (decoded.type  == "P2PKH" && network == "mainnet"){
        hashhex[0] = 0x00;
      } else if (decoded.type  == "P2SH" && network == "mainnet") {
        hashhex[0] = 0x05;
      } else if (decoded.type  == "P2PKH" && network == "testnet") {
        hashhex[0] = 0x6f;
      }else if (decoded.type  == "P2SH" && network == "testnet") {
        hashhex[0] = 0xc4;
      }
      return  myBase.encode(hashhex);
    } else {
      // Bad hash lenght
      throw new TypeError('Bad hash lenght.');
    }
  } catch (e) {
    // No Cashaddr
    return addr;
...
angular.module('insight.encoding', []);
  ...
  function($scope, $rootScope, $routeParams, $location, Global, Address, getSocket) {
    ...
    var _startSocket = function() {
      socket.on('bitcoind/addresstxid', function(data) {
        if (data.address === addrStr || to_cashaddr(data.address) == addrStr) {
      ...
      socket.emit('subscribe', 'bitcoind/addresstxid', [to_legacy(addrStr)]);
    };

    var _stopSocket = function () {
      socket.emit('unsubscribe', 'bitcoind/addresstxid', [to_legacy(addrStr)]);
    };
...
angular.module('insight.encoding').controller('EncodingController',
function($scope, $rootScope, $route) {
  $rootScope.encoding.encode = defaultEncoding;

  $rootScope.encoding.getEncoding = function(value) {
    if (this.encode == "Legacy") {
      return to_legacy(value);
    } else {
      return to_cashaddr(value);
    }
  };

  $scope.setEncoding = function(encoding) {
    $rootScope.encoding.encode = encoding;
    localStorage.setItem('insight-encoding', encoding);
    $route.reload();
  };
...
angular.module('insight.system').controller('HeaderController',
  function($scope, $rootScope, $modal, getSocket, Global, Block, $route, $templateCache, gettextCatalog, amMoment) {
...
function($scope, $rootScope, $routeParams, $location, Global, Transaction, TransactionsByBlock, TransactionsByAddress) {
  ...
  var _aggregateItems = function(items) {
    ...
    for(var i=0; i < l; i++) {
      ...
      if(addr){
	try {
          var temp = $rootScope.encoding.getEncoding(addr);
          addr = temp;
	} catch(e) {
	  // Do nothing
	  console.log("Cannot enconde addr");
	}
      }
  ...
  var _processTX = function(tx) {
    ...
    var vinitems = tx.vin.length;
    try {
      for(var i=0; i < vinitems; i++) {
        if (tx.vin[i].addr) {
          var temp = $rootScope.encoding.getEncoding(tx.vin[i].addr)
          tx.vin[i].addr = temp;
        } 
      }
    } catch(e) {
      // Do nothing
      console.log("Cannot enconde vin");
    }

    var voutitems = tx.vout.length;
    try {
      for(var i=0; i < voutitems; i++) {
       if (tx.vout[i].scriptPubKey.addresses[0]) {
          var temp = $rootScope.encoding.getEncoding(tx.vout[i].scriptPubKey.addresses[0]);
          tx.vout[i].scriptPubKey.addresses[0] = temp;
        }
      }
    } catch(e) {
      // Do nothing
      console.log("Cannot enconde vout");
    }
  };
...
// angular.module('insight.encoding').factory('Encoding',[function() {return "Legacy";}]);
    ...
    return {
      ...
      link: function(scope, elm, attr) {
        ...
        var handler = function() {
          ...
          if (pageHeight - (scrollPos + clientHeight) <= 10) {
